...
class ManufacturerDashboardController extends Controller
{
    ...
    public function production_analytics()
    {
        ...
        $totalItemsProcessed = $productionLines->count();
        $totalCompletedItems = $productionLines->where('current_stage', 'completed')->count();
        $totalFailedItems = $productionLines->where('current_stage', 'failed')->count();
        ...
        $stageCounts = [
            'raw_materials' => $productionLines->where('current_stage', 'raw_material')->count(),
            'manufacturing' => $productionLines->where('current_stage', 'assembling')->count(),
            'quality_control' => $productionLines->where('current_stage', 'quality_control')->count(),
            'distribution' => 0, // Not used yet
            'retail' => 0, // Not used yet
            'completed' => $totalCompletedItems,
            'failed' => $totalFailedItems,
        ];

        $stageLabels = array_keys($stageCounts);
        $stageData = array_values($stageCounts);

        // 3. Stage Duration Analysis (dummy data)
        ...
        // 4. Production Rate Over Time (dummy data)
        $productionRateLabels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
        $productionRateData = [100, 120, 110, 130, 140, 150];

        // 5. Failure Trends Over Time (dummy data)
        $failureTrendLabels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
        $failureTrendData = [5, 3, 4, 2, 1, 0];
